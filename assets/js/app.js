async function fetchContent() {
  ...
  function renderBooks(books) {
    ...
    books.items.forEach((book) => {
      ...
      if (Array.isArray(book.suitableFor)) {
      ...
      // hero book shortcut: use first book
  ...
  function renderTestimonials(testimonials) {
    $("#testimonials-subtitle").textContent = testimonials.subtitle;
    ...
    setupTestimonialSlider(track, testimonials.items.length);
  }
  ...
  function setupTestimonialSlider(track, count) {
    if (count <= 1) {
      $all(".slider-btn").forEach((btn) => (btn.style.display = "none"));
      return;
    }
    ...
    $(".slider-btn.prev").onclick = () => {
      index = (index - 1 + count) % count;
      update();
    };
  
    $(".slider-btn.next").onclick = () => {
      index = (index + 1) % count;
      update();
    };
